...
func Init(levelStr string, asJSON bool) error {
	initOnce.Do(func() {
		...
		// zap.AddCaller() - Показывает откуда был вызван метод логгера
		// zap.AddCallerSkip(2) - показывает только 2 уровня дерева, откуда был вызов, а абсолютный путь
...
// buildProductionEncoderConfig - Возвращает конфиг для логгера
